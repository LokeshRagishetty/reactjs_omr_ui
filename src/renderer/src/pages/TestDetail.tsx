...
export function TestDetail() {
  ...
  const [step, setStep] = useState('upload') // upload | converting | running | complete
  ...
  async function loadData() {
    try {
      const testData: any = await getTest(id!)
      const settingsData: any = await getSettings(user!.uid)
      setTest(testData)
  ...
  async function handleFileSelect(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      console.error(err)
      addToast('Error processing PDF: ' + err.message, 'error')
      setStep('upload')
    }
  }
  ...
  async function handleRunPython() {
    try {
      ...
      console.error(e)
      addToast('Error running Python: ' + e.message, 'error')
    }
  }
    ...
    <div className="max-w-4xl mx-auto p-6">
      <div className="mb-6 flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-gray-500">Date: {test.date} • Template: {test.templateFolder}</p>
        </div>
        <Link to="/" className="text-blue-600 hover:text-blue-800 text-sm font-medium">&larr; Back to Dashboard</Link>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 mb-6">
        ...
        {step === 'converting' && (
          <div className="text-center py-12">
            ...
            <p className="text-sm text-gray-500">Reading {pagesCount > 0 ? pagesCount : '...'} pages</p>
          </div>
        )}
            ...
            <div className="overflow-x-auto border border-gray-200 rounded-lg max-h-96">
              <table className="w-full text-sm text-left">
                <thead className="bg-gray-50 sticky top-0">
                  <tr>
                    {csvData.length > 0 && Object.keys(csvData[0]).map(k => (
                      <th key={k} className="px-4 py-2 font-medium text-gray-600">{k}</th>
                    ))}
                  </tr>
                </thead>
